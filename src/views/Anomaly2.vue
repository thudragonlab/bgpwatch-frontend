...
import { getEventlistByDate } from '@/api/AnomalyApi'
...
export default {
  ...
  methods: {
    ...
    getEventlistByDate(utcTimeStamp){
      getEventlistByDate(...utcTimeStamp).then(res => {
    ...
    selectDateZone(newDateZone, utcTimeStamp) {
      ...
      this.getEventlistByDate(utcTimeStamp)
    },
    getDateString(date) {
      // console.log(date)
    ...
    timeFilter(utcTimeStamp) {
      ...
      this.getEventlistByDate(_utcTimeStamp)
    },
    download() {

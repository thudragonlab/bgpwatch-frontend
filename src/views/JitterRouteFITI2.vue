      ...
      >
        <div class="btn-box">
          <el-button
            ...
            :key="item.value"
            @click.self="selectASN"
            :data-v="item.value"
            ><span @click.self="selectASN" :data-v="item.value">{{ item.label }}</span></el-button
          >
        </div>
...
<script>
import { JitterLineOption2 } from '@/templates/chart/lineTemplate'
...
export default {
  ...
  data() {
    return {
      JitterLineOption: JitterLineOption2,
  ...
  methods: {
    ...
    selectASN(e) {
      this.ASValue = e.target.dataset.v
